...
db = client.chrome
collect = db.polyA
altPolyA = db.altPolyA

book = xlsxwriter.Workbook("Nrich_Proximal_Distrib.xlsx")
...
polyAList = []
altcursor = altPolyA.find({},{'_id' : 0 , 'proximal' : 1})
for record in altcursor:
	polyAList.append(record['proximal'])
mcursor = collect.find({'_id': {'$in' : polyAList}})
