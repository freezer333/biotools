...
public:
    ...
    string G4:: toString(bool print_overlaps) {
        stringstream out;
        out << "{";
...
vector<G4> find(string sequence, short min_tetrads, short min_score) {
    vector <G4> raw_g4s;
   ...
   // std::cerr << "candidates" << std::endl;
    ...
    //std::cerr << "g4s" << std::endl;
    return g4s;
}
string makeJSON(vector<G4> g4s, bool overlaps){
    stringstream out;
    out << "{\"results\" : [";
    int k = 0;
    for ( vector<G4>::iterator git = g4s.begin(); git != g4s.end(); ++git) {
        out << git->toString(overlaps) ;
        if ( ++k != g4s.size()) {
            out << "," << endl;
        }
        else {
